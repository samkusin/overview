    ...
    if (!strcmp(componentName, "loadout"))
    {
        ...
        if (type[0])
        {
            ...
            if (!strcmp(type, "player"))
            {
                ...
                entityService.entityGroupTable(component::Loadout::kGroupId_Player).addDataToEntity(entity);
            }
            else if (!strcmp(type, "small_starship"))
            {
                data->type = component::Loadout::Type::kSmallShip;
                entityService.entityGroupTable(component::Loadout::kGroupId_SmallShip).addDataToEntity(entity);
            }
            else
            {
    ...
    else if (!strcmp(componentName, "party"))
    {
        ...
        if (type[0])
        {
            ...
            if (!strcmp(type, "player"))
            {
                ...
                entityService.entityGroupTable(component::Party::kGroupId_Player).addDataToEntity(entity);
            }
            else if (!strcmp(type, "small_starship"))
            {
                data->type = component::Party::Type::kSmallShip;
                entityService.entityGroupTable(component::Party::kGroupId_SmallShip).addDataToEntity(entity);
            }
            else
            {
                //  unsupported loadout type
