...
MessageDelegateHandle MessagePublisher::queue
(
...
MessageDelegateHandle MessagePublisher::on
(
...
void MessagePublisher::remove(MessageDelegateHandle delegateHandle)
{
...
MessageStream MessagePublisher::acquireStream(MessageStream&& publishTargetStream)
{
...
void MessagePublisher::dispatch(MessageStream& stream)
{
    MessageHeader hdr;
    do
    {
        ...
        else
        {
            ...
            if (it != _classDelegates.end() && it->first == hdr.classId)
            {
                ...
                while (entry != head)
                {
                    ...
                    entry->del(hdr.classId, reader, 0);
...
void MessagePublisher::obtainDelegateEntry(DelegateEntry* entry)
{
    ++entry->refcnt;
}

void MessagePublisher::releaseDelegateEntry(DelegateEntry* entry)
{
    if (entry->refcnt > 0)
    {
        --entry->refcnt;
