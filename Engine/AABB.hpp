...
namespace ckm {
    ...
    {
        typedef _Point                       point_type;
        typedef AABB2<_Point>                 this_type;
        typedef typename _Point::value_type  value_type;
        ...
        /// @param length   Sets min to -length and max to +length
        ///
        AABB2(value_type length);
        /// Constructor
        /// @param halfDim  Radius of the box
        ///
        AABB2(const point_type& halfDim);
        /// Constructor
        /// @param min  Min point of the box
        /// @param max  Max point of the box
        ///
        AABB2(const point_type& min, const point_type& max);
        /// Checks if the box is valid.  The box could be flat (planar), or even
        /// a line if the min/max points share common coordinates.
        ///
        /// @return False if the box has a zero size
        ...
        /// @return True if the this box lies within the supplied box
        ///
        bool inside(const AABB2& box) const;
        /// Determines if this box contains a point
        /// @param  point The point to test
        /// @return True if the this box contains the supplied point
        ///
        ...
        /// @return True if this box lies outside the supplied box
        ///
        bool outside(const AABB2& box) const;
        /// Determines if a box intersects this box
        /// @param  box The box to test
        /// @return True if the box intersects this box
        ///
        bool intersects(const AABB2& box) const;
        ...
        */
        /// @param  box The box to test
        /// @return True if the box equals this box
        bool operator==(const AABB2& box) const;
        /// @param  box The box to test
        /// @return True if the box does not equal this box
        bool operator!=(const AABB2& box) const;
        /// @param  pt  Point to offset existing AABB
        /// @return Reference to the offsetted AABB
        ///
        AABB2& operator+=(const point_type& pt);
        /// Zeros out the box
        ///
        void clear();
        /// Calculates the center point of our box
        ...
        /// @param verts An array of vertices to be filled in
        ///
        void generateVertices(std::array<point_type, 4>& verts);
        /// Rotates, generating a new AABB bounding the rotated box
        /// @param mtxRot Rotation and Translation matrix
        ///
        template<class _Matrix>
    ...
    template<class _Point>
    AABB2<_Point>::AABB2(const point_type& halfDim)
    {
        clear();
        min -= halfDim;
        max += halfDim;
    ...
    template<class _Point>
    AABB2<_Point>::AABB2(const point_type& min, const point_type& max) : min(min), max(max)
    {
    }

    template<class _Point>
    AABB2<_Point>::operator bool() const
    {
        return min != max;
    }

    template<class _Point>
    typename AABB2<_Point>::point_type AABB2<_Point>::dimensions() const
    {
        return max - min;
    }

    template<class _Point>
    bool AABB2<_Point>::intersects(const AABB2<_Point>& box) const
    {
        return !outside(box);
    }

    template<class _Point>
    bool AABB2<_Point>::operator==(const AABB2<_Point>& box) const
    {
        return min != box.min || max != box.max;
    }
    ...
    template<class _Point>
    typename AABB2<_Point>::point_type AABB2<_Point>::center() const
    {
        return (min + max)/2.f;
    }
    
    template<class _Point>
    AABB2<_Point>& AABB2<_Point>::operator+=(const point_type& pt)
    {
        min += pt;
        max += pt;
        return *this;
    ...
    template<class _Point>
    AABB2<_Point> operator+(const AABB2<_Point>& box, const typename AABB2<_Point>::point_type& off)
    {
        AABB<_Point> result;
        result.min = box.min + off;
        result.max = box.max + off;
    ...
    template<class _Point>
    AABB2<_Point>::AABB2(value_type length)
    {
        min = point_type(-length, -length);
        max = point_type(length, length);
    }

    template<class _Point>
    void AABB2<_Point>::clear()
    {
        min.x = min.y = 0;
        max.x = max.y = 0;
    }
    ...
    template<class _Point>
    bool AABB2<_Point>::inside(const AABB2<_Point>& box) const
    {
        return (min.x >= box.min.x && max.x <= box.max.x &&
                min.y >= box.min.y && max.y <= box.max.y);
    }
    ...
    template<class _Point>
    bool AABB2<_Point>::contains(const point_type& pt) const
    {
        return (pt.x >= min.x && pt.x <= max.x &&
                pt.y >= min.y && pt.y <= max.y);
    }
    ...
    template<class _Point>
    bool AABB2<_Point>::outside(const AABB2<_Point>& box) const
    {
        return (min.x > box.max.x || box.min.x > max.x ||
                min.y > box.max.y || box.min.y > max.y);
    }
    ...
    void AABB2<_Point>::generateVertices(std::array<point_type, 4>& verts)
    {
        ...
        verts[1].y = min.y;
        verts[2].x = max.x;     // v2 (+x,+y)
        verts[2].y = max.y;
        verts[3].x = min.x;     // v3 (-x,+y)
        verts[3].y = max.y;
    }

    template<class _Point>
    template<class _Matrix>
    void AABB2<_Point>::rotate(const _Matrix& mtxRot)
    {
        std::array<point_type, 4> box;
        generateVertices(box);
    ...
    template<class _Point>
    AABB2<_Point>& AABB2<_Point>::merge(const AABB2<_Point>& aabb)
    {
        if (aabb.min.x < min.x)
            min.x = aabb.min.x;
        if (aabb.min.y < min.y)
    ...
    template<class _Point>
    typename AABB2<_Point>::value_type AABB2<_Point>::area() const
    {
